...
class Order extends DB
{
    ...
    { //in admin order
        $stm = $this->db->prepare('SELECT o.id,o.time,o.status,o.paymentid,u.username FROM ordered o INNER JOIN user u ON u.id = o.user_id
    ...
    {
        $stm = $this->db->prepare("SELECT *,code.code_name FROM " . ' products_orders,code WHERE products_orders.order_id=:id and products_orders.order_id=code.order_id');
    ...
    function insert($payload, $cart)
    {
        try {
            ...
            $stm = $this->db->prepare('INSERT INTO ' .
                self::tableName . '(user_id,status,paymentid,extra_address)
                 VALUES(:user_id, :status, :paymentid, :extra_address)');
            $stm->execute(array(
                'user_id' => $user_id,
                'status' => 0,
                'paymentid' => $notes,
        ...
        try {
            ...
            if ($status == '2' && $check_status != 0) { //status = 2 la da nhan dc hang
                foreach ($listProducts_QuantityOrder as $product) {
                    $selled = $product['config'] + $product['quantity'];
                    $quantity = $product['quantity_product'] - $product['quantity'];
                    $stm = $this->db->prepare('UPDATE  product' .
                        ' SET quantity_product=:quantity,config=:config WHERE id= :id');
