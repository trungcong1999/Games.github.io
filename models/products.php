...
require_once('C:/xampp/htdocs/Games/db.php');
...
class Product extends DB implements Iproduct
{
    ...
    function getListGarena(){
        $sql="SELECT * FROM `product` WHERE name like '%garena%'";
        $result=$this->db->prepare($sql);
        $result->execute();
    }
    ...
    function getGarena($offset,$count){
        $stm=$this->db->prepare("SELECT * FROM `product` WHERE name like 'garena%' ORDER BY name asc limit $offset,$count");
        $stm->execute();
        return $stm->fetchAll();
    }
    function getSteam($offset,$count){
        $stm=$this->db->prepare("SELECT * FROM `product` WHERE name like 'steam%' ORDER BY name asc limit $offset,$count");
        $stm->execute();
        return $stm->fetchAll();
    }
    ...
    {
        try {
            $stm = $this->db->prepare("SELECT * FROM " .  self::tableName . ' WHERE 1 ORDER BY config DESC LIMIT 5');
            $stm->execute();
            return $stm->fetchAll();
        } catch (\Throwable $e) {
            echo $e;
    ...
    function insert($payload, $srcs, $srcOfContent)
    {
        try {
            ...
            $stm = $this->db->prepare('INSERT INTO ' .
                self::tableName . '(name,price,brand_id,keyword,short_desc,discount)
                            VALUES(:productName,:price,:brand_id,:keyword,:short_desc,:discount)');
            $stm->execute(array(
    ...
    function update($payload)
    {
        try {
            ...
            ,short_desc=:short_desc,discount=:discount WHERE id = :id');
            $stm->execute(array(
                ':productName' => $productName,
                ':price' => $price,
                ':brand_id' => $brand_id,
